Shader "Hidden/Locked/.poiyomi/Poiyomi Toon URP World/aa2f17256d5405f4bace4c0b529dd9b5"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void PoiShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				#endif
				#ifdef _LIGHTINGMODE_FLAT
				poiLight.finalLighting = poiLight.directColor * attenuation * shadowAttenuation + poiLight.indirectColor;
				if (1.0)
				{
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[5.0].xy, float4(1,1,0,0));
				if (0.0)
				{
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.8,0.8,0.8,1).rgb, 0.0);
