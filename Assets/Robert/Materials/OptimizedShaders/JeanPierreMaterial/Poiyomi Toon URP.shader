 ...
 #define OPTIMIZER_ENABLED 
			...
			in float4 mainTexture)
			{
				...
				if (4.0 == 1 || 4.0 == 2)
				{
					if (4.0 == 1)
					{
						poiLight.direction = mul((float3x3)unity_ObjectToWorld, float4(0,1,0,1)).xyz;
					}
					if (4.0 == 2)
					{
			...
			float3 GetWorldReflections(float3 reflDir, float3 worldPos, float roughness, float2 screenUV)
			{
				...
				#endif
				#if POI_PIPE == POI_URP
				return GlossyEnvironmentReflection(reflDir, worldPos.xyz, roughness, 1, screenUV);
				#endif
				return 0;
			}
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				if (0.0 == 1)
				{
					...
					specular2 = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(1,1,1,1).rgb, 0.0) * poiLight.occlusion * attenuation * 1.0;
					if (poiFragData.toggleVertexLights)
					{
						#if defined(VERTEXLIGHT_ON)
						for (int index = 0; index < 4; index++)
						{
							...
							vSpecular2 += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(1,1,1,1).rgb, 0.0) * poiLight.occlusion * 1.0;
						}
						#endif
					}
				}
			...
			in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods)
			{
				#ifdef MOCHIE_PBR
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (!skipShading)
				{
					...
					PoiLightingDependantModules(poiLight, poiFragData, poiMesh, poiCam, poiMods);
					...
					UNITY_LOOP for (uint lightIndex = 0; lightIndex < min(URP_FP_DIRECTIONAL_LIGHTS_COUNT, POI_MAX_VISIBLE_LIGHTS); lightIndex++)
					{
						...
						PoiLightingDependantModules(poiLightAdd, poiFragData, poiMesh, poiCam, poiMods);
