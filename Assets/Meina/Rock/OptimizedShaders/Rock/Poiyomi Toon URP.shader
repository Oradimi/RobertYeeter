Shader "Hidden/Locked/.poiyomi/Poiyomi Toon URP/21bfebc201e53474492069a19decd49d"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void PoiCalculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				#endif
				#ifdef _LIGHTINGMODE_TEXTURERAMP
				float2 rampUVs = poiLight.lightMap + 0.0;
				if (1 > 1)
				{
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.2144469,0.2144469,0.2144469,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.2144469,0.2144469,0.2144469,1).a;
				#else
