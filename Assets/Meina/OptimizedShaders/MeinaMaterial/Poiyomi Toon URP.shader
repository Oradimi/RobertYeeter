Shader "Hidden/Locked/.poiyomi/Poiyomi Toon URP/a5bc88297a2376a4babd7a125e20d034"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			#if defined(POI_MATCAP0) || defined(POI_MATCAP1) || defined(POI_MATCAP2) || defined(POI_MATCAP3)
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
				getMatcapUV(matcapUV, float4(0,0,0,0).xy, 1.0, 0.0, float4(0,0,0,0).xy, 0.0, 1.0, normal0, poiCam, poiLight, poiMesh, 1.0, matcapALD);
				...
				float matcapSmoothness = 0.4;
				if (1.0)
				{
					#if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
					matcapSmoothness *= POI2D_SAMPLER_PAN(_MatcapMask, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0))[1.0];
					#endif
				}
				...
				blendMatcap(poiLight, poiFragData, poiMods, 0.0, 0.0, _MatcapMultiply, _MatcapReplace, 0.0, 0.0, matcap, matcapMask, 0.0, 0.0, 0.0, 2.0, matcapALD);
				#endif
			}
			#endif
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				float glowInTheDarkMultiplier0 = calculateGlowInTheDark(0.0, 0.2, 1.0, 0.2, 1.0, 0.0, poiLight);
				#ifdef POI_GRABPASS
